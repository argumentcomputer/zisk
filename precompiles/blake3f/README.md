...
| `tools/regen-blake3f-pk.sh` | Generate proving key + verify setup | ~25 min (32 cores, `--setup-jobs 8 --recursive-jobs 8`) |
...
Only `pil2-proofman` is needed as a sibling repo (v1.0.0-alpha dropped
pil2-compiler / pil2-proofman-js from the setup flow: PIL compilation uses
the npm-published `pil2com`, and proving-key setup is the Rust
`pil2-stark-setup` crate wrapped by `cargo-zisk-dev proofman-setup`):

```bash
# From the parent directory of the zisk repo:
git clone https://github.com/0xPolygonHermez/pil2-proofman.git
(cd pil2-proofman && git checkout v1.0.0-alpha && npm i)   # npm i provides pil2com
```
...
```bash
# From zisk repo root (same invocation `cargo-zisk-dev proofman-setup
# compile-pil` wraps; calling pil2com directly avoids needing a prebuilt
# cargo-zisk-dev):
NODE_OPTIONS="--max-old-space-size=65536" \
  ../pil2-proofman/node_modules/.bin/pil2com pil/zisk.pil \
  -I pil,../pil2-proofman/pil2-components/lib/std/pil,state-machines,precompiles \
  -o pil/zisk.pilout -u tmp/fixed -O fixed-to-file -O no-proto-fixed-data
```
...
cp target/release/cargo-zisk target/release/cargo-zisk-dev \
   target/release/ziskemu target/release/libziskclib.a $HOME/.zisk/bin
...
The v1.0.0-alpha setup is the Rust `pil2-stark-setup` (parallelizable with
`--setup-jobs`/`--recursive-jobs`), which replaced pil2-proofman-js's
`main_setup.js` — wall time dropped from 1 hour+ to ~25 min on 32 cores.
Size on disk: ~13 GB of core setup files plus ~36 GB of regenerable
`*.consttree` files written during setup. On a GPU build, the first prove
(or a `check-setup` on the gpu binary) additionally materializes ~40 GB of
`*_gpu` const/consttree variants, so a fully warmed key directory lands
around ~89 GB.

```bash
# From zisk repo root (Rust setup; writes $HOME/.zisk/provingKey directly):
cargo-zisk-dev proofman-setup setup \
  --airout ./pil/zisk.pilout \
  --build-dir $HOME/.zisk \
  --fixed-dir tmp/fixed \
  --stark-structs ./state-machines/starkstructs.json \
  --recursive --setup-jobs 8 --recursive-jobs 8
