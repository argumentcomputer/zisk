...
impl<F: PrimeField64> PrecompileMemInputs for Blake3fSM<F> {
    ...
    ///
    /// Total: 20 memory operations per compression.
    fn generate<P: MemProcessor>(
        ...
        if !only_counters {
            ...
            let message: [u64; 8] =
                data[message_offset..message_offset + PARAM_CHUNKS_MESSAGE].try_into().unwrap();
            let aux: [u64; 2] =
