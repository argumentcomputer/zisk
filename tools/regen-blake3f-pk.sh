#!/usr/bin/env bash
#
# Regenerate the full Zisk pilout + PIL helpers + release build + proving key.
...
#   4.5 Pre-flight checks (cargo tests + blake3-host execute) — gates step 5
#   5. Generate proving key (~30-45 min) + install to ~/.zisk/provingKey
#   6. Verify setup
#
# Usage:
#   tools/regen-blake3f-pk.sh              # full run (skips step 1 if tmp/fixed exists)
#   tools/regen-blake3f-pk.sh --with-fixed # force regenerate fixed data
#   tools/regen-blake3f-pk.sh --cpu-only   # build with --features cpu-only (force CPU)
#   tools/regen-blake3f-pk.sh --skip-tests # skip pre-flight tests before step 5
#   tools/regen-blake3f-pk.sh --skip-pk    # stop after step 4 (no proving key)
#
# Assumes:
...
if [[ $FORCE_FIXED -eq 1 ]] || [[ ! -d tmp/fixed ]] || [[ -z "$(ls -A tmp/fixed 2>/dev/null)" ]]; then
    step_begin "Step 1/6: Generate fixed data (arith + binary tables)"
    ...
    echo "▶  Step 1/6: Skipped (tmp/fixed already populated; use --with-fixed to force)"
fi

# ── Step 2 ────────────────────────────────────────────────────────────────
step_begin "Step 2/6: Compile PIL → pil/zisk.pilout"
...
# ── Step 3 ────────────────────────────────────────────────────────────────
step_begin "Step 3/6: Regenerate pil_helpers/traces.rs from pilout"
(
    cd ../pil2-proofman
...
# ── Step 4 ────────────────────────────────────────────────────────────────
step_begin "Step 4/6: Rebuild zisk (release${BUILD_FEATURES:+ }${BUILD_FEATURES}) + install binaries"
...
# Run before the ~45-min proving-key gen so a constraint/witness bug is
# caught in minutes, not after the expensive step.
if [[ $SKIP_TESTS -eq 0 ]]; then
    step_begin "Step 4.5/6: Pre-flight checks (tests + execute)"
    ...
    echo
    echo "▶  Step 4.5/6: Skipped (--skip-tests) — not recommended before step 5"
fi

# ── Step 5 ────────────────────────────────────────────────────────────────
if [[ $SKIP_PK -eq 1 ]]; then
    echo
    echo "▶  Step 5/6: Skipped (--skip-pk)"
    echo "▶  Step 6/6: Skipped"
    total=$((SECONDS - SECONDS_START))
    printf "\nTotal time: %dm %ds (no proving key regenerated)\n" $((total / 60)) $((total % 60))
    exit 0
fi

step_begin "Step 5/6: Generate proving key (~30-45 min, patience)"
...
# ── Step 6 ────────────────────────────────────────────────────────────────
step_begin "Step 6/6: Verify setup (cargo-zisk check-setup -a)"
...
echo
echo "════════════════════════════════════════════════════════════════════════"
printf "✓  All 6 steps complete in %dm %ds\n" $((total / 60)) $((total % 60))
echo "════════════════════════════════════════════════════════════════════════"
echo
echo "Next steps:"
