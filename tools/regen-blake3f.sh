...
#
# Proving-key regeneration is intentionally split out — it takes ~25 min
# (Rust pil2-stark-setup, 32 cores) and shouldn't run on every iteration.
# After this script completes, run `tools/regen-blake3f-pk.sh` to
# (re)generate the proving key.
#
# Usage:
...
#   - ../pil2-proofman exists, checked out at the tag pinned in Cargo.toml
#     (v1.0.0-alpha) and npm-installed at its repo root (provides pil2com).
#     pil2-compiler / pil2-proofman-js are no longer needed: PIL compilation
#     goes through the npm-installed pil2com, and proving-key setup is the
#     Rust pil2-stark-setup (via cargo-zisk-dev proofman-setup).
...
if [[ ! -x ../pil2-proofman/node_modules/.bin/pil2com ]]; then
    echo "ERROR: pil2com not found. Run 'npm install' in ../pil2-proofman." >&2
    exit 1
fi
...
# The build auto-detects CUDA at build time (ziskbuild/build.rs sets
...
# Same invocation `cargo-zisk-dev proofman-setup compile-pil` wraps, minus the
# chicken-and-egg on cargo-zisk-dev itself (it may not be built yet).
...
# v1.0.0-alpha bin set (mirrors ziskup's BINS + the staticlib the asm
# emulator links against, if built).
cp target/release/cargo-zisk \
   target/release/cargo-zisk-dev \
   target/release/ziskemu \
   "$HOME/.zisk/bin/"
if [[ -f target/release/libziskclib.a ]]; then
...
echo
echo "Next steps:"
echo "  tools/regen-blake3f-pk.sh         # generate proving key (~25 min)"
echo
