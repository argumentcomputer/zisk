...
# regenerable const-tree files (`*.consttree`, ~71 GB) and the GPU variants
# (`*_gpu`, unused on the CPU-only runner). That is ~3 GB gzipped instead of
# ~48 GB. CI regenerates the const-trees after download with
# `cargo-zisk check-setup --proving-key <dir> -a` (exactly how `ziskup` itself
# populates them).
#
# Prerequisites
# -------------
